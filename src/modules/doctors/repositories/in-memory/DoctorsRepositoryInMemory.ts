...
class DoctorsRepositoryInMemory implements IDoctorsRepository{
    ...
    async filterDoctors(name?: string, crm?: string, landline?: string, cellPhone?: string): Promise<Doctor[]> {
    ...
    async findById(id: string): Promise<Doctor> {
        const doctor = this.doctors.find((doctor) => doctor.id === id && !doctor.deleted_at);
        return doctor;
    }

    async findByCrm(crm: string): Promise<Doctor> {
        const doctor = this.doctors.find((doctor) => doctor.crm === crm && !doctor.deleted_at);
        return doctor;
    }

    async findByCellPhone(cellPhone: string): Promise<Doctor> {
        const doctor = this.doctors.find((doctor) => doctor.cellPhone === cellPhone && !doctor.deleted_at);
        return doctor;
    }

    async findByLandline(landline: string): Promise<Doctor> {
        const doctor = this.doctors.find((doctor) => doctor.landline === landline && !doctor.deleted_at);
        return doctor;
    }
